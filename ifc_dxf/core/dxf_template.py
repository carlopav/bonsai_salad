...
def _ensure_dim_style(doc, scale_factor, unit_scale=1.0):
    ...
    Prefers the template's own style -- 'dimensions_imperial' where the template
    carries one, 'dimensions_metric_m' otherwise -- and uses its sizes and arrow
    ...
    fallback with our own standard sizes, converted to drawing units.
    ...
    for name in (_DIM_STYLE_IMPERIAL, _DIM_STYLE_NAME):
        if name in doc.dimstyles:
            _make_dim_style_annotative(doc, name)
            return name

    # Paper-space sizes, stated in metres and converted to drawing units --
    # the annotation scale multiplies these.
